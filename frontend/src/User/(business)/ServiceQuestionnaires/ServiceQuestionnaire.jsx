...
export default function EditServiceQuestionnairesForm({ token, business }) {
  ...
  const { data: questionnaireData, isLoading: loadingQ } =
    useFetchServiceQuestionnaireQuery(id, {
      skip: !token || !id,
    });

  const [updateServiceQuestionnaire, { isLoading: saving }] =
    useUpdateServiceQuestionnaireMutation();
  ...
  useEffect(() => {
    if (questionnaireData) {
      ...
      const clonedQuestions = questionnaireData.additional_questions_form
        ?.length
        ? JSON.parse(
            JSON.stringify(questionnaireData.additional_questions_form),
          )
        : [
            {
              text: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlert({
        type: "danger",
        message: "Failed to update questionnaire. Please try again.",
      });
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Select
              ...
              isRequired={true}
              isDisabled={true}
              options={servicesOffered.map((service) => ({
                value: service,
                label: service,
              }))}
            />
