...
export default function UserDashboard({ page, token, user }) {
  ...
  const renderTab = () => {
    switch (page) {
      case "profile":
        // User account tab
        return <Profile token={token} user={user} setAlert={setAlert} />;
      case "business":
        // User account settings tab
        return <Business token={token} user={user} setAlert={setAlert} />;
      case "banking":
      ...
      case "client-services":
        // Client services tab
        return (
          <ClientServices token={token} role={user?.role} business={business} />
        );
      case "service":
        // Service details tab
