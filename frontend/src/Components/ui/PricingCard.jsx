  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: plan.featured ? "var(--color-accent)" : "transparent",
      }}
    >
      <h3
        className={`font-Intro mb-4 ${plan.featured ? "text-accent text-3xl font-bold" : "text-white text-2xl font-bold"}`}
      >
        {plan.name}
      </h3>
      {plan.description && (
      ...
      <div className="mb-6">
        <div className="flex items-end lg:gap-3">
          ...
          {plan.discountPct && (
            <div className="lg:ml-2 px-2 py-1 text-xs rounded-full bg-accent text-white">
              -{plan.discountPct}%
            </div>
          )}
        </div>
          ...
          {plan.features.map((f, i) => (
            <li key={i} className="flex items-center gap-3 mt-4 text-white/70">
              <span className="w-5 h-5 rounded-full flex items-center justify-center text-black text-xs">
                {plan.featured ? (
                  <IoCheckmarkCircle className="text-accent" size={20} />
                ) : (
      ...
      <div className="mt-auto pt-2 text-center">
        ...
        >
          Subscribe{" "}
          <IoIosArrowForward className="inline-block ml-2 mb-1" size={20} />
        </button>
      </div>
    </div>
  );
